...
struct TLS{
	...
	//struct TLS * shared_tls[NUM_THREADS];
	//int num_shared;
};
struct TLS  tls_array[NUM_THREADS];
//**********************************************************************
//************************TLS STRUCT END********************************
//**********************************************************************
...
//**********************************************************************
struct page{
	unsigned int address;//start address of the page
	int ref_count;//counter of shared pages -> increment on clone and decrement on write
};
//**********************************************************************
//************************TLS STRUCT END********************************
//**********************************************************************
...
void tls_handle_page_fault(int sig, siginfo_t *si, void *context)
{
 	...
 	struct TLS * faulty_tls;
 	 for(bb=0; bb<NUM_THREADS; bb++){
 		 if(tls_array[bb].tid==pthread_self()){
 		 	faulty_tls=&tls_array[bb];
 	 }
 	}
 	/*for (bb=0; bb<tls_cnt; bb++){
 		for (cc=0;cc<tls_array[bb].page_num; cc++)
 		if(tls_array[bb].pages[cc]->address==p_fault){
 			tls_destroy();
 			pthread_exit(NULL);
 			gg++;
 			break;
 		...
 		for(cc=0;cc<faulty_tls->page_num;cc++){
	if (faulty_tls->pages[cc]->address == p_fault) {
		tls_destroy();
	       pthread_exit(NULL);
	       break;
	}
...
void tls_protect(struct page *p)
{
  if (mprotect((void *) p->address, page_size, 0)) {
    fprintf(stderr, "[%lu] tls_protect: could not protect page\n");
    exit(1);
	} 
}
//**********************************************************************
...
void tls_unprotect(struct page *p)
{
  if (mprotect((void *) p->address, page_size, PROT_READ | PROT_WRITE))
	{
    	fprintf(stderr, "[%lu] tls_unprotect: could not unprotect page\n");
		exit(1); 
	}
}
//**********************************************************************
...
int tls_create(unsigned int size){
	//create a local storage area for a thread
	//that can hold at least size bytes
	//printf("In tls_create \n");
	if (initialized==0){
		tls_init();
	}
	if (size<=0){
	...
	int i=0;
	for (i=0; i<NUM_THREADS; i++){//tls_cnt may want to be NUM_THREADS
		if(pthread_self()==tls_array[i].tid){
			return -1;
		}
	}
	...
	sem_wait(&mutex);
	/*printf( "tls_cnt: %x", tls_cnt);
	printf("\n");
	printf( "tls_array: %x", &(tls_array[tls_cnt]));
	printf("\n");*/
	struct TLS * self = &tls_array[tls_cnt];
	tls_cnt++;//add error checking for wrap around later if need be
	//current_tls = self;
	self->size=size;
	self->tid=pthread_self();
	self->page_num=(size/page_size+1);
	if((size%page_size)==0){
		self->page_num--;
	}
	self->pages = (struct page **)calloc(self->page_num, sizeof(struct page));
	i=0;
	for (i=0; i<self->page_num; i++){
		...
		/*printf( "self: %x", self );
		printf("\n");
		printf( "pages[%d]: %x", i, self->pages[i] );
		printf("\n");*/
		self->pages[i]=p;
		//fprintf(stderr, "[%lu] TOM\n" );
		//printf("leaving tls_create tls#: %d \n", tls_cnt-1);
	}
	sem_post(&mutex);
	//sem_destroy(&mutex);
...
//************************TLS WRITE START*******************************
//**********************************************************************
int tls_write(unsigned int offset,//postion in local storage area (LSA) of current thread that read bytes are written to
 unsigned int length,//amount of bytes to read at postion *buffer
  char* buffer){//the location at which the thread reads length bytes
	struct TLS *current_tls;
	...
	for (i=0; i<NUM_THREADS; i++){
		if(pthread_self()==tls_array[i].tid){
			current_tls=&tls_array[i];//global that points to current tls
			tid_found=1;
			break;
		}
	}
	...
	if (offset+length>current_tls->size){
		sem_post(&mutex1);
		return -1;//ERROR if it is asked to read past the current thread's LSA limits OR the current thread has no LSA 
	//i.e., offset +  length >  size of LSA
	//Don't need to worry about the buffer not having length bytes
	}
	//sem_wait(&mutex);
	for (i=0; i<current_tls->page_num; i++){

		tls_unprotect(current_tls->pages[i]);
	}
	...
	int idx=0;
	char * dst;
	// LOGIC TO CHECK IF WRITE IS VALID AND HOW MANY PAGES IT SPANS
	for (cnt = 0, idx = offset; idx < (offset + length); ++cnt, ++idx) {
	   struct page *p, *copy;
	   unsigned int pn, poff;
	   pn = idx / page_size;
	   poff = idx % page_size;
	   p = current_tls->pages[pn];
	   if (p->ref_count > 1) {
	...
	for (i=0; i<current_tls->page_num; i++){
		...
		tls_protect(current_tls->pages[i]);
	}
	sem_post(&mutex1);
	//sem_post(&mutex);
	return 0;//On success
...
//*************************TLS READ START*******************************
//**********************************************************************
int tls_read(unsigned int offset,//postion within the current thread's LSA that you read length bytes
	unsigned int length,//read length bytes of current thread @ offset
	char *buffer){//location that length bytes are written to
	...
	int idx=0;
	char * src;
	// LOGIC TO CHECK IF READ IS VALID AND HOW MANY PAGES IT SPANS
	for (cnt = 0, idx = offset; idx < (offset + length); ++cnt, ++idx) {
	    struct page *p;
	    unsigned int pn, poff;
	    pn = idx / page_size;//last page reached
	    poff = idx % page_size;//where on the last page you start
	    p = current_tls->pages[pn];
	    src = ((char *) p->address) + poff;//specific address corresponding to the last page touched
	    buffer[cnt] = *src;
  	}
...
//************************TLS DESTROY START*****************************
//**********************************************************************
int tls_destroy(){//frees currently executing thread's LSA
	...
	//fprintf(stderr, "[%lu] IN DESTROY\n" );
	//printf("IN DESTROY\n");
	i=0;
	for (i=0; i<current_tls->page_num; i++){//clean up pages
		current_tls->pages[i]->ref_count=0;
		munmap((void *)current_tls->pages[i]->address,page_size);
	}
	//fprintf(stderr, "[%lu] IN DESTROY2\n" );
	//printf("IN DESTROY2\n");
	for(i=0; i<current_tls->page_num; i++){
		free(current_tls->pages[i]);
	}
	//fprintf(stderr, "[%lu] IN DESTROY3\n" );
	//printf("IN DESTROY3\n");
	current_tls->tid=2222;
	current_tls->size=2222;
	current_tls->page_num=2222;
	//sem_destroy(&mutex);
	//free(curr_tls);
	//may need to clean tls_array as well
...
//************************TLS CLONE START*******************************
//**********************************************************************
int tls_clone(pthread_t tid){//clones LSA of thread with thread id tid
	//tls_write is the only function that can make a specific part of the LSA be private
	//shared memory space using Copy on Write methodology
	int i=0;
	int k = 0;
	...
	//struct TLS * target_tls;
	for (i=0; i<NUM_THREADS; i++){
		if(tid==tls_array[i].tid){
			k=i;
			tid_found=1;
			target_tls=&tls_array[i];
			break;
		}
	}
	if(tid_found==0){
	...
	fprintf(stderr, "[%lu] tls_clone() 1\n", pthread_self());

	struct TLS * self = &tls_array[tls_cnt];
	self->tid=pthread_self();
	self->size=target_tls->size;
	self->page_num=target_tls->page_num;
	self->pages = (struct page **)calloc(self->page_num, sizeof(struct page));
	fprintf(stderr, "[%lu] tls_clone() 2\n", pthread_self());

	i=0;
	fprintf(stderr, "[%lu] tls_clone() page_num: %d\n", pthread_self(), self->page_num);

	fprintf(stderr, "[%lu] tls_clone() target_tls %d\n", pthread_self(), target_tls->size);
	fprintf(stderr, "[%lu] tls_clone() target_tls %d\n", pthread_self(), target_tls->page_num);
	fprintf(stderr, "[%lu] tls_clone() target_tls %x\n", pthread_self(), target_tls->pages);
	for (i=0; i<self->page_num; i++){
		struct page *p;
		fprintf(stderr, "[%lu] tls_clone() %x\n", pthread_self(), target_tls);
		fprintf(stderr, "[%lu] tls_clone() %x\n", pthread_self(), target_tls->pages[i]);
		fprintf(stderr, "[%lu] tls_clone() %d\n", pthread_self(), target_tls->pages[i]->ref_count);
		target_tls->pages[i]->ref_count++;
		fprintf(stderr, "[%lu] tls_clone() 4\n", pthread_self());
		p=target_tls->pages[i];//(unsigned int)target_tls->pages[i];
		fprintf(stderr, "[%lu] tls_clone() 5\n");
		fprintf(stderr, "[%lu] tls_clone() 5 %x\n", pthread_self(), self->pages);
		fprintf(stderr, "[%lu] tls_clone() 5 %x\n", pthread_self(), self->pages[i]);
		self->pages[i]=p;
		fprintf(stderr, "[%lu] tls_clone() 6", pthread_self());
	}
	tls_cnt++;

	fprintf(stderr, "[%lu] tls_clone() 3", pthread_self());
	sem_post(&mutex1);
...
GENERAL NOTES:
--------------
tls_read and tls_write temporarily can unprotect the LSA itself and a target thread

getpagesize() // to get the page size (library call)
Page size is normally 4096 bytes
LSA has two pages of space. the first page is private while the second page is shared inbetween threads to cut down on copying

More than two threads can use tls_clone meaning that all these threads share memory stored at the same location. It isn't 
until one of the threads call tls_write that it gets it own LSA space.

HINTS:
------
Need to create memory space that threads cannot freely access. Use mmap
mmap -> allocates memory space in multiples of page size.
The mmap() function establishes a mapping between a process' address space and a file or shared memory object.

void *mmap(void *addr, size_t len, int prot, int flags,
    int fildes, off_t off);

//create mapping at address space pa for len long bytes
pa=mmap(addr,
 len,
  prot,//read write permission PROT_NONE is don't allow either.
   flags, // decide if changes are private or shared
    fildes, //memory object represented by the file descriptor fildes
     off);//offset within file descriptor
http://pubs.opengroup.org/onlinepubs/7908799/xsh/mmap.html -> man page for mmap



mprotect -> allows one to protect/unprotect memory locations of a thread
use this for tls_write and tls_read


The two pages in the LSA may not be in contiguous memory as one may be shared while the other is private

When a thread tries to access a protected LSA , 
a signal handeler should throw a SIGSEGV causing a 
termination of the thread that accessed this protected memory (by calling pthread_exit)
this cannot be done for each SIGSEGV though, so we need to distinguish between our call and the OS's call to SIGSEGV
Do this by looking into sigaction and find how the struct siginfo_t can help
*/
...
int get_address()
{
	...
	for(ii = 0; ii<128; ii++)
	{
		if (tls_array[ii].tid == pthread_self())
		{
			storage = &tls_array[ii];
			break;
		}
	}
	if(storage == NULL)
